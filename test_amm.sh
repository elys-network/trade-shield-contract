...
sleep 2
instantiate_hash=$(extract_txhash elysd tx wasm instantiate 1 '{"epoch_cycle_interval": "1"}' --from=treasury --label "Contract" --chain-id=elystestnet-1 --gas=auto --gas-adjustment=1.3 -b=sync --keyring-backend=test --no-admin -y)  
sleep 2
...
sleep 2
elysd tx wasm exec $addr '{"create_order": {"order_amm_routes" : [], "order_price" : {"base_denom": "uusdc","quote_denom" : "uatom" ,"rate" : "1"}, "order_type" : "stop_loss", "order_target_denom" : "uatom", "order_source_denom" : "uusdc"}}' --from treasury --gas-prices 0.25uelys --gas auto --gas-adjustment 1.3 -b sync -y  --keyring-backend=test --chain-id=elystestnet-1 --amount=200uusdc
sleep 2
balance_before=$(elysd q bank balances $addr)
sleep 2
elysd tx wasm exec $addr '{"process_order": {}}' --from treasury --gas-prices 0.25uelys --gas auto --gas-adjustment 1.3 -b sync -y  --keyring-backend=test --chain-id=elystestnet-1
sleep 2
balance_after=$(elysd q bank balances $addr)
sleep 2
